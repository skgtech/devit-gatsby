...
const MorePhotosCTA = () => (
  <Col md={4} css={css`position: relative;`}>
    <img css={css`width: 100%;`} src="/assets/images/press/background.svg" alt="More images"/>
    <a css={css`
      position: absolute;
      top: 0;
      left: 0;
      ...
      color: #57585a;
    `} rel="noopener noreferrer" target="_blank" href="https://www.facebook.com/pg/DEVitConf/photos/">Browse all photos <i class="fas fa-long-arrow-alt-right"></i></a>
  </Col>
)
...
const Logo = ({ title, svg, png, png2x, png3x }) => (
  <Col md={6}>
    ...
    `}>{title}</div>
    <div>
      <a rel="noopener noreferrer" target="_blank" href={svg}>SVG</a> or
      <a rel="noopener noreferrer" target="_blank" href={png}>PNG</a>
      (
        <a rel="noopener noreferrer" target="_blank" href={png2x}>2x</a>,
        <a rel="noopener noreferrer" target="_blank" href={png3x}>3x</a>
      )
    </div>
  </Col>
);
...
export default ({ title, description, image }) => {
  return (
    <Layout
      ...
      <Header />
      <Grid>
        <Block css={css`margin-bottom: 72px`}>
          <DarkBlockHeading>
            Some copy samples
          </DarkBlockHeading>
          <DarkLeading>
        ...
        <Block css={css`margin-bottom: 72px`}>
          ...
          <p>
            <a rel="noopener noreferrer" target="_blank" href="https://drive.google.com/file/d/1rKfM-XDaBOLDJVYZUPArh3lij9CksSIB/view">In Greek</a>
          </p>
        </Block>
        <Block css={css`margin-bottom: 72px`}>
          <DarkBlockHeading>
        ...
        <Block css={css`margin-bottom: 72px`}>
          ...
          <DarkLeading>
            Photos are copyrighted with <a rel="noopener noreferrer" target="_blank" href="https://creativecommons.org/licenses/by-sa/4.0/">CC BY-SA 4.0</a> and it is required that credits will follow them on each usage: Dimitris Tsakiris, <a rel="noopener noreferrer" target="_blank" href="https://www.facebook.com/Dimitris.Tsakiris.Photo/">www.dimitristsakiris.com</a>
          </DarkLeading>
        </Block>
      </Grid>
      <Footer/>
